...
* Before writing a functional test, check whether an existing `*IT` — or an existing `@Test` method inside it — already builds the fixture you need, and extend that instead of adding a new one. A functional test is a scenario: write as few `@Test` methods as the scenario allows, one if it allows one, so the fixture is paid once. `@Order` does not share a fixture and is not a substitute. See the scenario rule in `okf/testing/strategy.md`.
